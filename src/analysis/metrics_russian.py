...
def toxicity(human, llm=None):
    ...
    human_toxicity = convert_to_scalar(run_hf_model(human, "s-nlp/russian_toxicity_classifier", 'toxicity'))
    if llm is None:
        return human_toxicity
    llm_toxicity = convert_to_scalar(run_hf_model(llm, "s-nlp/russian_toxicity_classifier", 'toxicity'))

...
class BERTAlignModel(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        if not self.is_finetune:
            ...
            print()
            print("I cannot calculate F1 score!!!")
            print()
...
# print(get_align_score(["home", "home"], ["house", "home"]))


if __name__ == "__main__":
    mod_dir = '/shared/0/projects/research-jam-summer-2024/data/english_only/prompting_results_clean/'
    fname = "wildchat_subset_en_2k_prompting_Mixtral-8x7B-Instruct-v0.1.jsonl"
    ...
    human_turns = []
    llm_turns = []
    for item1,item2 in zip(data['human_turn_3'], data['llm_turn_3']):
        if len(item1) == 0 and len(item2) > 0:
            human_turns.append("[no response]")
            llm_turns.append(item2)
        elif len(item1) > 0 and len(item2) == 0:
            human_turns.append(item1)
            llm_turns.append("[no response]")
        elif len(item1) == 0 and len(item2) == 0:
            human_turns.append("[no response]")
            llm_turns.append("[no response]")
        else:
    ...
    print(len(human_turns), len(llm_turns))
    align_score = get_align_score(human_turns, llm_turns)
    data['align_score'] = align_score

    data.to_csv("alignscore_results_"+fname.split(".")[0] + ".csv")

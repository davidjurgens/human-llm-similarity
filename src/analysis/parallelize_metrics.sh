...
metric_groups=("end,lexical,capitalization,grammar,punctuation,pos,sbert,topic" "sentiment,formality,politeness,toxicity,readability,semantic")
...
for i in "${!metric_groups[@]}"; do
  ...
  if [ ! -f "${output_directory}/${metric_groups[$i]}.jsonl" ]; then
    python analysis/metrics.py \
      ...
      --output_path ${output_directory}/${metric_groups[$i]}.jsonl \
